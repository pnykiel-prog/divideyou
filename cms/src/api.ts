const TOKEN_KEY = 'divideyou_cms_token';
...
async function request<T = any>(method: string, path: string, body?: any): Promise<T> {
  ...
  if (!res.ok) throw new ApiError(res.status, data?.error_message || 'Request failed');
  return data as T;
}
...
export const date = (d?: string | null) => (d ? new Date(d).toLocaleDateString() : '—');
